...
model = XGBRegressor(n_estimators= 194,max_depth=3,learning_rate= 0.2,min_child_weight= 4,subsample=1,colsample_bytree= 0.9,gamma= 0)#TCs
#model = XGBRegressor(n_estimators= 171,max_depth=10,learning_rate= 0.2,min_child_weight= 10,subsample=0.5,colsample_bytree= 1.0,gamma= 0.0)#SAs
#model = XGBRegressor(n_estimators= 184,max_depth=4,learning_rate= 0.2,min_child_weight= 1,subsample=0.5,colsample_bytree= 0.7,gamma= 0.0)#FQs

# Define stratified cross-validation object based on column 'A'
kf = KFold(n_splits=10, shuffle=True, random_state=42)
# 读取Excel文件
# Split the dataset with the current random_state
train_data_split = spt2.split_excel("D:\ML\Combined\Combined_Models.xlsx",
                                   #'SDZ', 'SCP', 'SMT',
                                      'OTC', 'TC', 'CTC',
                                       #'ENR', 'CIP', 'NOR',
                                   210)
